...
//	    		tableList.add(table);
	    		...
	    		if (result != null) {
	    			for(int i = 0; i < result.size(); i++) {
	    				...
	    				if ("ET_ORDER_HEADERS_OUT".equals(result.get(i).getTableName())) {
	    					if ( !result.get(i).getRows().isEmpty()){
		    					for (int j = 0; j < result.get(i).getRows().size(); j++) {
		    						insert("SapIfRecvSalesOrderMapper.insertSapIfRecvORDER_HEADERS_OUT", result.get(i).getRows().get(j));
		    					}
	    					}	    					
...
//        						logger.debug("========== KNB1 =================> " + list.toString());
				...
				String fileName = svrNm + "_getCustomer_" + sdf.format(startDate).substring(0, 8);
				createLog(fileName, sb.toString());
			}
		}
    }    
