...
<c:import url="../import/frameTop.jsp">
	<c:param name="progcd" value="Interface" />
</c:import>
...
<!--                </li> -->
		...
		<div id="ctm_sech_wrap" >
			<ul class="sech_ul">
				...
				<li class="sech_list">
					<label> upload   &nbsp;&nbsp;&nbsp;&nbsp; </label>
					<input type="button" id="btnSapUpload" name="btnSapUpload" value="btnSapUpload" />
				</li>
			</ul>
		</div>
		...
		<div id="ctm_sech_wrap" >
			<ul class="sech_ul">
				<li class="cls"></li>
				<li class="sech_list">
					<label> SAP GET ATTACHE FILE</label>
					<input type="text" id="IV_VBELN" name="IV_VBELN" value="0612100492"/>
					<input type="text" id="IV_DOCNO" name="IV_DOCNO" value="000000000114"/>
					<a href='#' onclick="fileDownloadEx();">btnSapATTACHE</a>
				</li>
			</ul>
		</div>
	</form>
...
--%>
	...
	$(function() {
		...
		$("#btnSapUpload").bind("click", function () {
			var params = {'url':'/sample/uploadSAPSample.do'};    // 실제 엑셀 파일을 업로드하고 엑셀 파일을 읽을 경로
			popupOpen('/sample/UploadPopup.do', 'excelupload', params, function(data){  // 엑셀 업로드 공통 팝업
				;
			});
		});
	});

	function fileDownloadEx() {
		//var url ="<c:url value="/common/downloadBBSAttach.do"/>";
		var url ="<c:url value="/sample/getSAPATTACHESample.do"/>";
		url += "?IV_VBELN=" + $("#IV_VBELN").val() + "&IV_DOCNO=" + $("#IV_DOCNO").val();
