...
<!-- 			<input type="text" name="SEQ"  id="SEQ"      value="" />   -->
			...
			<table class="pop_tblForm">
				<colgroup><%@ page language="java" contentType="text/html; charset=UTF-8"	pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ taglib prefix="s" uri="http://www.springframework.org/tags"%>
			...
			<table class="pop_tblForm">
			    ...
			    <tr class="idTest">
					...
					<td class="medium_td" colspan="5">
						...
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<button type="button" class="btn btn-primary" id="btn_search" style="width:115px;">아이디찾기</button>	
					</td>
				</tr>
				...
				<tr>
				    ...
				    <td colspan="3">
					    <div style="display:inline-flex;">
						    ...
						    <input type="text" id="M_PERSON" name="M_PERSON" style="width:40px; text-align: right; margin:0 -5px" value="0" maxlength="3" class="withComma"/>&nbsp;명 &nbsp;
						    ...
						    <input type="text" id="G_PERSON" name="G_PERSON" style="width:40px; text-align: right;" value="0" maxlength="3" class="withComma"/>명 &nbsp;
						    <input type="hidden" id="G_PERSON_HIDDEN" name="G_PERSON_HIDDEN">
						    
						    <p style="font-size: 12px; font-weight: bold; opacity:80%;"><mark>&nbsp;비라운딩</mark></p>
						    <input type="text" id="N_PERSON" name="N_PERSON" style="width:40px; text-align: right;" value="0" maxlength="3" class="withComma"/>명 &nbsp;
						    ...
						    <input type="text" id="K_PERSON" name="K_PERSON" style="width:40px; text-align: right;" value="0" maxlength="3" class="withComma"/>명 &nbsp;
						    ...
						    <input type="text" id="I_PERSON" name="I_PERSON" style="width:40px; text-align: right;" value="0" maxlength="3" class="withComma"/>명
						    <input type="hidden" id="I_PERSON_HIDDEN" name="I_PERSON_HIDDEN">
						</div>
					</td>
				    <th>총 인 원</th>
					<td>
						<input type="text" class="cmc_txt withComma" id="TOT_PERSON" name="TOT_PERSON" value="0" maxlength="3" style="width:60px; text-align: right; margin:0 5px" />명
					</td>
				</tr>
				...
				<tr>
				    ...
				    <td>
						...
						<button type="button" class="btn btn-primary openPop" id="insertPickGbn" style="width:50px;">등록</button>
					</td>
				</tr>
				...
				<tr>
				    ...
				    <td colspan="3">
				    <div style="display:inline-flex;" >
						<input type="checkbox"  name="RND_CHG_YN1" id="RND_CHG_YN1" style="margin:0 5px">토요일 오전 &nbsp;&nbsp;
					...
					<td>
						...
						<button type="button" class="btn btn-primary openPop" id="btn_create" style="width:60px;">생성</button>
					</td>
				</tr>
				...
				<tr>
				    ...
				    <th>입금관리</th>
				    <td>
				    	<button type="button" class="btn btn-success" id="btn_pay"  style="width:160px; opacity:70%; margin: 0 20px">입금관리</button>
				    </td>
				    ...
				    <td>
					    ...
					    <button type="button" class="btn btn-primary openPop" id="changeStatus" style="width:60px;">상태변경</button>
				    </td>
				</tr>
				<tr>
...
// 		$("#btn_create").hide(); // 인보이스 발행일자 생성 버튼
	...
	$('#M_PERSON, #G_PERSON, #N_PERSON, #K_PERSON, #I_PERSON').on("keyup" , function(){
		...
		if($("#MEM_GBN").val() != '02'){ //01 멤버 02 일반회원 04에이전시
...
// 			}
		...
		if(mem_gbn == "01" || (mem_gbn == "02" && fn_empty(agn_cd))){
			...
			var late_check_in = $("#LATE_CHECK_IN").val();
			if(fn_empty(late_check_in)){
				alert("EARLY CHECK IN을 선택해주세요.");
				return false;
			}
			var late_check_out = $("#LATE_CHECK_OUT").val();
			if(fn_empty(late_check_out)){
				alert("LATE CHECK OUT을 선택해주세요.");
				return false;
			}
...
<%-- 						    <s:message code='reservation.roundY'/> --%>일반
			...
			<div class="ctu_g3_wrap" style="width:100%; float:left; padding-top:10px; margin-bottom:6px; ">
				<div class="pop_grid_top_wrap">
					<div class="ct_grid_top_left" style="align:left; width:10%; float:left; margin-bottom:-6px; padding-top:1px; padding-bottom:-6px; ">
						<button type="button" class="pbtn_default" id="btn_com_add" style="align:left">동반자정보</button>
					</div>
					<div class="ct_grid_top_right" style="right; width:87%; float:left; margin-bottom:-6px; padding-top:1px; padding-bottom:-6px; ">
						<button type="button" class="btn btn-default" id="btn_List_addRow" style="align:right" ><s:message code='button.addRow'/></button>
		            	<button type="button" class="btn btn-default" id="btn_List_delRow" style="align:right" ><s:message code='button.delRow'/></button>
...
$(function() {
	...
	$('#MEM_GBN').change(function() {
		if($(this).val() == '01'){ // 01 멤버, 02 일반, 03 교민, 04 에이젼시
			$("#AGN_GB").val("");
			$("#AGN_CD").val("");
			isDisabled(false);
			$("#M_PERSON"  ).attr("disabled",false);
...
// 			}
	...
	$('#CHK_IN_DT, #CHK_OUT_DT').on("change" , function(){
		fn_hdngGbnList();
		fn_addHdngGbnList();
	});
	
	function fn_hdngGbnList(){
		if ($("#MEM_GBN").val() == '')return;
		
		var url = "/reserve/hdngGbnList.do";
		var param = {"MEM_GBN"    : $("#MEM_GBN").val()
		           , "CHK_IN_DT"  : $("#CHK_IN_DT").val().replaceAll(".", "")
		           , "CHK_OUT_DT" : $("#CHK_OUT_DT").val().replaceAll(".", "")
		           };
		fn_ajax(url, true, param, function(data, xhr){
			if(!fn_empty(data.hdngGbnList)){
				$("#HDNG_GBN").find("option").remove();
				$("#HDNG_GBN").append("<option value='' selected> -선택- </option>");
				
				for (var i = 0; i < data.hdngGbnList.length; i++) {
					$("#HDNG_GBN").append("<option value="+data.hdngGbnList[i].CODE+">"+data.hdngGbnList[i].CODE_NM+"</option>");
				}
			}else{
				alert("상품이없습니다. 관리자페이지에서 등록후 이용해주세요.");
			}
	...
	function fn_addHdngGbnList(){
		if ($("#MEM_GBN").val() == '')return;
		
		var url = "/reserve/hdngGbnList.do";
		var param = {"MEM_GBN"    : "02"
		           , "CHK_IN_DT"  : $("#CHK_IN_DT").val().replaceAll(".", "")
		           , "CHK_OUT_DT" : $("#CHK_OUT_DT").val().replaceAll(".", "")
		           };
		fn_ajax(url, true, param, function(data, xhr){
			if(!fn_empty(data.hdngGbnList)){
				...
				for (var i = 0; i < data.hdngGbnList.length; i++) {
					$("#ADD_HDNG_GBN").append("<option value="+data.hdngGbnList[i].CODE+">"+data.hdngGbnList[i].CODE_NM+"</option>");
				}
			}else{
				alert("상품이없습니다. 관리자페이지에서 등록후 이용해주세요.");
			}
