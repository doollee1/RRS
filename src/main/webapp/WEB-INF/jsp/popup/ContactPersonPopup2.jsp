...
<%
/**
 * @Name : ContactPersonPopup2
 * @Description : Lead에서 사용하는 ContactPersonPopup, Cust_cd 입력가능
 */
%>
<div id="ContactPersonPopup2">
	<!--- 검색버튼 ---->
		...
		<table class="tab_top_search" width="100%">
				<tr>
					<td class="small_td"><s:message code='customer.Customer'/></td>
		      		<td><p><input type="text" id="S_CUST_CD" name="S_CUST_CD" style="width:180px;"></p></td>
					<td class="small_td"><s:message code='customer.lastName'/></td>
		      		<td><p><input type="text" id="S_LAST_NM" name="S_LAST_NM" style="width:180px;"></p></td>
		      		<td class="small_td"><s:message code='customer.firstName'/></td>
		      		<td><p><input type="text" id="S_FIRST_NM" name="S_FIRST_NM" class="cmc_txt"  style="width:180px;"></p></td>
				</tr>
		</table>
	</form>
	</div>
...
$(function() {
	$('#ContactPersonPopup2').dialog({
		...
		buttons: {
			'<s:message code='button.confirm'/>': { 
				...
				click: function() {		
					...
					popupClose($('#ContactPersonPopup2').data('pid'));
		...
		open: function(e, ui) {
			$('#ContactPersonPopup2 .pop_grid_wrap').height($(this).height() - $('#ContactPersonPopup2.popup_search').outerHeight(true) - 70);
			...
			$('#grid_ContactPersonMG').jqGrid('setGridParam', {
				ondblClickRow: function(rowid, iRow, iCol, e) {
					...
					//parent.$('#PS_NO').val(p_rtnData[0].CODE);
					//parent.$('#PS_NM').val(p_rtnData[0].FULL_NM);	
					//parent.$('#DESC').val(p_rtnData[0].SEARCH_TERM);					
			...
			var onSearchFld = '#S_CUST_CD, #S_LAST_NM, #S_FIRST_NM';
...
function grid_ContactPersonMG_Load() {
	...
	var colModel = [
					...
					{name:'CUST_CD',index:'CUST_CD',width:120},
					...
					{name:'SEARCH_NM',index:'SEARCH_NM',width:150,hidden:true},
