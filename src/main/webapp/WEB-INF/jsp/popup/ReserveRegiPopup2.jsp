...
<!--            <input type="text" name="SEQ"  id="SEQ"      value="" />   -->
			...
			<table class="pop_tblForm" >
					...
					<td>
						...
						<select id="AGN_CD" name="AGN_CD" class="cmc_combo" style="width:90px;">
							...
							<c:forEach var="i" items="${agency}">
								<option value="${i.CODE}" class="AGN_CD_${i.REF_CHR1} AGN_HIDDEN">${i.CODE_NM}</option>
							</c:forEach>
						</select>
					</td>
				</tr>
				...
				<!-- 도착항공기편 -->
				<tr>
					<th><s:message code='reservation.arrFlightDt'/></th>
					<td>
						<input type="text" class="cmc_txt text-center" id="FLIGHT_IN_DT"     name="FLIGHT_IN_DT" data-type="date" style="width:145px;"/>
					</td>
				...
				<!-- 출발항공기편 -->
				<tr>
					<th><s:message code='reservation.depFlightDt'/></th>
					<td>
						<input type="text" class="cmc_txt text-center" id="FLIGHT_OUT_DT"     name="FLIGHT_OUT_DT" data-type="date" style="width:145px;"/>
					</td>
...
$(function() {
	 ...
	 * -----------------------------------------------------*
	 *******************************************************/
	$('#REQ_TEL_NO, #REQ_ENG_NM, #HDNG_GBN, #ADD_HDNG_GBN').on("change",function(key){
		...
		if(vflag == 'detail'){
			/* 회원구분이 에이전시가 아닐 때만 */
			if ((memGbn != '03') && (memGbn != '04') && (memGbn != '05')) {
				/* 예약자 변경사항(전화번호/영문명) - 예약자만 변경 */
				if(selec_id == 'REQ_TEL_NO'){
					$("#reserveGrid").jqGrid("setCell", ids[0], 'COM_TEL_NO', selec_val);
					$("#reserveGrid").jqGrid("setCell", ids[0], "STATUS_V", "U");
				}
				else if(selec_id == 'REQ_ENG_NM'){
					$("#reserveGrid").jqGrid("setCell", ids[0], 'COM_ENG_NM', selec_val);
					$("#reserveGrid").jqGrid("setCell", ids[0], "STATUS_V", "U");
				}
				/* 기본상품변경시 예약자 패키지상품 변경 */
				/* 예약자가 일반이고 동반자도 일반일때   */
				else if(selec_id == 'HDNG_GBN'){
					$("#reserveGrid").jqGrid("setCell", ids[0], "HDNG_GBN", selec_val);
					$("#reserveGrid").jqGrid("setCell", ids[0], "STATUS_V", "U");
					for(var i = 0; i < ids.length; i++){
						var gbn = $("#reserveGrid").jqGrid("getCell",ids[i], "NUM_GBN");
						 if(gbn == '02'){
							 $("#reserveGrid").jqGrid("setCell",ids[i], "HDNG_GBN", selec_val);
						 }
					}
				}
				/* 예약자가 멤버이면서 동반자가 일반 일때 추가패키지 상품 */
				else if(selec_id == 'ADD_HDNG_GBN'){
					for(var i = 0; i < ids.length; i++){
						var gbn = $("#reserveGrid").jqGrid("getCell",ids[i], "NUM_GBN");
						 if(gbn == '02'){
							 $("#reserveGrid").jqGrid("setCell",ids[i], "HDNG_GBN", selec_val);
		...
		if(ids.length > 0){
			for(var i = 0; i < ids.length; i++){
				if(selec_id == 'FLIGHT_IN_DT' || selec_id == 'FLIGHT_OUT_DT'){
					$("#reserveGrid").jqGrid("setCell",ids[i] , selec_id, selec_val.replaceAll(".",""));
				}
				else{
					$("#reserveGrid").jqGrid("setCell",ids[i] , selec_id, selec_val);
				}
	...
	function fn_dataSet(data){
		...
		if(!fn_empty(data.PRC_STS)){
			if(data.PRC_STS == "08" || data.PRC_STS == "09" || data.PRC_STS == "10"){
				...
				//체크인 체크아웃 도착항공일자 출발항공일자 달력 비활성화
				$("#CHK_IN_DT, #CHK_OUT_DT, #FLIGHT_IN_DT, #FLIGHT_OUT_DT").attr("readonly",true);
				$("#CHK_IN_DT, #CHK_OUT_DT, #FLIGHT_IN_DT, #FLIGHT_OUT_DT").datepicker("disable").removeAttr("disable");
			}
		}
		...
		if (data.MEM_GBN == '03') {
			$(".AGN_CD_2").removeClass("AGN_HIDDEN");
		}else if (data.MEM_GBN == '04') {
			$(".AGN_CD_1").removeClass("AGN_HIDDEN");
		}else if (data.MEM_GBN == '05') {
			$(".AGN_CD_3").removeClass("AGN_HIDDEN");
		}
	...
	$('#CHK_IN_DT').change(function() {
		...
		$("#FLIGHT_IN_DT").val($("#CHK_IN_DT").val());
		$("#FLIGHT_IN_DT").change();
	});
	...
	$('#CHK_OUT_DT').change(function() {
		$('#ROOM_TYPE').val("");
		$('#FLIGHT_OUT_DT').val($('#CHK_OUT_DT').val());
		$('#FLIGHT_OUT_DT').change();
	...
	$('#AGN_GB').change(function() {
		fn_chgAgnCd($(this).val());
	});
	 
	function fn_chgAgnGb(memGbn) {
		/* 에이전시 select 셋팅*/
		if(memGbn == '03') {
			$("#AGN_GB").val("1").prop("selected", true);			
		} else if (memGbn == '04') {
			$("#AGN_GB").val("2").prop("selected", true);
		} else if (memGbn == '05'){
			 $("#AGN_GB").val("3").prop("selected", true);
		} else {
			$("#AGN_GB").val("").prop("selected", true);
		}
	}
	function fn_chgAgnCd(agnGb) {
		...
		if(agnGb == '1'){
			...
			$(".AGN_CD_2").removeClass("AGN_HIDDEN");
			$(".AGN_CD_3").addClass("AGN_HIDDEN");
		} else if(agnGb == '2'){
			$("#AGN_CD"  ).attr("disabled", false);
			$(".AGN_CD_1").removeClass("AGN_HIDDEN");
			$(".AGN_CD_2").addClass("AGN_HIDDEN");
			$(".AGN_CD_3").addClass("AGN_HIDDEN");
		} else if(agnGb == '3'){
			$("#AGN_CD"  ).attr("disabled", false);
			$(".AGN_CD_1").addClass("AGN_HIDDEN");
	...
	function saveReserveInfo(){
		...
		var obj   = { "USER_ID"         : $("#USER_ID").val()
					...
					, "CHK_OUT_DT"      : $("#CHK_OUT_DT").val().replaceAll(".","")
					, "ROOM_TYPE"       : $("#ROOM_TYPE").val()
					, "FLIGHT_IN_DT"    : $("#FLIGHT_IN_DT").val().replaceAll(".","")
					, "FLIGHT_IN"       : $("#FLIGHT_IN").val()
					, "FLIGHT_IN_HH"    : $("#FLIGHT_IN_HH").val()
					, "FLIGHT_IN_MM"    : $("#FLIGHT_IN_MM").val()
					, "FLIGHT_OUT_DT"   : $("#FLIGHT_OUT_DT").val().replaceAll(".","")
	...
	function isValidation(){
		...
		if((mem_gbn == "03" || mem_gbn == "04" || mem_gbn == "05")){
			if (fn_empty($("#AGN_GB").val()) || fn_empty($("#AGN_CD").val())) {
				alert("에이전시를 선택해주세요.");
				return false;
			}
		}
	...
	function createGrid(){
		...
		var colModel = [
						...
						, { name: 'FLIGHT_IN_DT'  , width : 84 , align: 'center', editable:true, editoptions:{maxlength:100}}
						...
						, { name: 'FLIGHT_OUT_DT' , width : 84 , align: 'center', editable:true, editoptions:{maxlength:100}}
	...
	$("#btn_com_add").on("click" , function(){
		...
		for(var i = 0 ; i < tot_person ; i ++ ){
			...
			if (i == 0 && (($("#MEM_GBN").val() != '03') && ($("#MEM_GBN").val() != '04') && ($("#MEM_GBN").val() != '05'))){ //
...
function cSearch(currentPage){
	...
	fn_ajax(url, true, param, function(data, xhr){
		...
		if((($("#MEM_GBN").val() != '03') && ($("#MEM_GBN").val() != '04') && ($("#MEM_GBN").val() != '05')) && data.result.length <= '0'){
			$("#btn_com_add").attr("disabled", false);
			$("#M_PERSON").attr("disabled",false);
			$("#G_PERSON").attr("disabled",false);
			$("#N_PERSON").attr("disabled",false);
